...
프록시의 일일 한도는 "최근 24시간"이 아니라 **정해진 시각에 리셋**됩니다. 429 본문에 `Resets at 2026-09-02T06:34:30Z` 처럼 찍히는 그 시각입니다. 기본값은 예전처럼 최근 24시간 합계이지만, Settings의 **Daily reset time** 에 `06:34` 같은 UTC 시각을 넣으면:

- 작업 표시줄의 `1d` 자리가 `today`(리셋 이후 사용량)로 바뀌고, 퍼센트 바도 그 값 기준이 됩니다.
- 툴팁과 통계 창에 **다음 리셋까지 남은 시간**이 나옵니다.
- 통계 표 맨 위에 `since reset` 행이 추가됩니다.

리셋 시각을 아직 모르면 비워 두세요. 한 번 429를 맞으면 그 본문에서 읽어 넣으면 됩니다. (여러 키를 같이 쓰는 경우 키마다 리셋 시각이 다를 수 있습니다.)
